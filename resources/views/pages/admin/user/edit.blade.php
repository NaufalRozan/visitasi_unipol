                ...
                <form action="{{ route('user.update', $user) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="role" id="role" class="form-control" required onchange="handleRoleChange()">
                            <option value="Universitas" {{ $user->role === 'Universitas' ? 'selected' : '' }}>Universitas</option>
                    ...
                    <!-- Ceklis Fakultas -->
                    <div class="form-group" id="units-checkboxes" style="display: {{ $user->role === 'Fakultas' ? 'block' : 'none' }}">
                    ...
                    <!-- Ceklis Prodi -->
                    <div class="form-group" id="sub_unit-checkboxes" style="display: {{ $user->role === 'Prodi' ? 'block' : 'none' }}">
                        <label for="sub_unit">Pilih Prodi</label>
                        @foreach ($unit as $fakultas)
                            <strong>{{ $fakultas->nama_unit }}</strong>
                            @foreach ($fakultas->sub_units as $prodi)
                                <div>
                                    <input type="checkbox" name="sub_units[]" value="{{ $prodi->id }}" class="sub_unit-checkbox unit-{{ $fakultas->id }}"
